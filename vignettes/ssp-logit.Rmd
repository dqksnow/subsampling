...
$$
where $u$ and $\psi$ are known functions depend on the distribution from the exponential family. For the 
binomial distribution, the log-likelihood function becomes
...
binary response variable. The full dataset size is $N = 1 \times 10^4$.
...
variable $U(0,1)$. The expected number of drawn samples are `n.ssp`. More details see @wang2019more.
...
After drawing subsample, `ssp.glm` utilizes `survey::svyglm` to fit the model
...
The object returned by `ssp.glm` contains estimation results and indices of the drawn
