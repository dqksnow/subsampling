...
are $d$-dimensional unknown coefficients. We define $\boldsymbol{\beta} =
(\boldsymbol{\beta}_0^\top, \boldsymbol{\beta}_1^\top, \ldots,
\boldsymbol{\beta}_K^\top)^\top$ as a $(K+1)d$-dimensional vector.
...
We introduce the usage of `ssp.glm` with simulated data. $X$ contains $d=3$
...
estimator. `LUC` and `uniform` are baseline methods. See @yao2023model for
details.
...
$0$. Without loss of generality, we set the category $Y=0$ as the baseline
category so that $\boldsymbol{\beta}_0=0$. The summation constraint
$\sum_{k=0}^{K} \boldsymbol{\beta}_k$ is also used in the subsampling method for
the purpose of calculating subsampling probability. These two constraints lead
...
the number of unique indices is smaller since we use `sampling.method =
'withReplacement'`. After fitting softmax model on subsample using the choosen
