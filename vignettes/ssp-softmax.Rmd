...
This vignette shows how to use `ssp.softmax`, which draws optimal subsample from
full data and fits softmax (multinomial) regression on the subsample. The
statistical theory and algorithms in this implementation can be found in the
...
number of drawn samples are $n.ssp$.
...
After drawing subsample, `ssp.softmax` uses `nnet::multinom` to fit the
...
the number of unique indices is less than 600 since we use `sampling.method = 'withReplacement'`. After fitting softmax model on subsample using the chosen
